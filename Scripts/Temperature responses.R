...
for(s in x:28) { #nrow(params)) {
  ...
  } else if(params[sp.num,]$Species == "Aulacorthum solani Brazil") { TR <- 292 # must use a different TR for this species b/c mortality rates must be set to data
  } else { TR <- 293 }
  params[sp.num,]$TR <- TR
  ...
  params[sp.num,]$sR0 <- abs(round(coef(R0.nls)[3], 2))
  ...
  # Nonlinear regression
  b.nls<- nls(Birth_Rate ~ bTopt*exp(-(T_K-Toptb)^2/(2*sb^2)), data=sp.data[1:7,],
  ...
  params[sp.num,]$sb <- abs(round(coef(b.nls)[3], 2))
  ...
  # Nonlinear regression
  if(params[sp.num,1] != "Aulacorthum solani Brazil" && params[sp.num,1] != "Eriosoma lanigerum Australia Yathroo" && params[sp.num,1] != "Hyperomyzus lactucae Australia Acton" && 
     params[sp.num,1] != "Aulacorthum solani US Ithaca" && params[sp.num,1] != "Aphis citricola Japan Chiba" &&
     params[sp.num,1] != "Toxoptera citricidus on Citrus aurantium Japan Chiba" && params[sp.num,1] != "Aphis gossypii Iran") {
    ...
    # Assign parameters
    if(coef(dJ.nls)[1] > 0.001) { params[sp.num,]$dJTR <- round(coef(dJ.nls)[1], 3)
    } else { params[sp.num,]$dJTR <- 0.0001*trunc(10000*coef(dJ.nls)[1]) } # For populations in which dJTR is really low, simply truncate at the 4th decimal point
    params[sp.num,]$AdJ <- round(coef(dJ.nls)[2], 0)
  } else { # For these populations, must set dJTR to data and fit AdJ via nls
    if(params[sp.num,1] == "Aulacorthum solani Brazil") { dJTR.test <- sp.data[2,"Juv_Mortality"] }
    if(params[sp.num,1] == "Eriosoma lanigerum Australia Yathroo") { dJTR.test <- sp.data[4,"Juv_Mortality"] }
    if(params[sp.num,1] == "Hyperomyzus lactucae Australia Acton") { dJTR.test <- sp.data[1,"Juv_Mortality"] }
    if(params[sp.num,1] == "Aulacorthum solani US Ithaca") { dJTR.test <- sp.data[3,"Juv_Mortality"] }
    if(params[sp.num,1] == "Aphis citricola Japan Chiba") { dJTR.test <- sp.data[2,"Juv_Mortality"] }
    if(params[sp.num,1] == "Toxoptera citricidus on Citrus aurantium Japan Chiba") { dJTR.test <- sp.data[3,"Juv_Mortality"] }
    if(params[sp.num,1] == "Aphis gossypii Iran") { dJTR.test <- sp.data[3,"Juv_Mortality"] }
  ...
  # Nonlinear regression
  if(params[sp.num,1] != "Aulacorthum solani Brazil") {
    ...
    # Assign parameters
    params[sp.num,]$dATR <- round(coef(dA.nls)[1], 3)
    params[sp.num,]$AdA <- round(coef(dA.nls)[2], 0)
  } else {
    dATR.test <- sp.data[2,"Adult_Mortality"] # For Aulacorthum solani in Brazil, must set dATR to data and fit AdA via nls
    dA.nls <- nls(Adult_Mortality ~ dATR.test*exp(AdA*(1/TR-1/T_K)), data=sp.data, start=list(AdA=params[sp.num,]$AdA))
    print(summary(dA.nls))

    # Assign parameters
    params[sp.num,]$dATR <- round(dATR.test, 3)
  ...
  else { for(j in nrow(sp.data):i) { if(sp.data[j,"Development"] != 0) { break } } # find the highest laboratory temperature at which the development rate is not zero
  ...
  if(word(params[sp.num,2],1) == "Brazil") { i <- 3 }
  if(word(params[sp.num,1],1,2) == "Apolygus lucorum") {
  ...
  if(error1 == FALSE) {
    ...
    if(params[sp.num,1] == "Clavigralla shadabi Benin") { params[sp.num,]$gTR <- round(gTR.test, 2) # For this species, it is useful to round at few decimal points
    } else { params[sp.num,]$gTR <- round(gTR.test, 3) }
    ...
    if(error2 == FALSE) {
      ...
      #
      # # If nls still fails for AL, assign TL to just above coldest laboratory temperature and try nonlinear regression for AL again
      # if(error3 == TRUE) {
      #   TL.test <- min(sp.data[,"T_K"]) + 2
      #   g.nls2 <- nls(Development ~ gTR.test*(T_K/TR)*exp(Ag.test*(1/TR-1/T_K))/(1+exp(AL*(1/TL.test-1/T_K))), data=sp.data[1:i,],
      #                 start=list(AL=params[sp.num,]$AL))
  ...
  #if(word(params[sp.num,2],1) == "Brazil" ||
  if(params[sp.num,2] == "UK Sand Hutton") { i <- 3 } # For these populations, exclude high temperatures at which development rate declines
