...
Sdata = read_csv("Temperature response parameters manuscript.csv")
if daily == True:
    Tdata = read_csv("Habitat temperature parameters daily.csv")
else:
    Tdata = read_csv("Habitat temperature parameters.csv")
...
while(True):
    ...
    Topt = spData["Toptg"] # optimum developmental temperature
    Tmax = spData["Tmaxg"] # maximum developmental temperature
    # mortality
    dJTR = spData["dJTR"]
    AdJ = spData["AdJ"]
    dATR = spData["dATR"]
