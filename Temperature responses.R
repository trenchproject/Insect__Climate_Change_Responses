...
# NLS (monotonic response)
r.mon <- nls(Development ~ xTR*T_K/TR*exp(A*(1/TR-1/T_K)), data=sp.data,
               start=list(xTR=0.01, A=5000))
summary(r.mon)
# Plot model fits
plot(sp.data$T_K, sp.data$Development)
...
# NLS for development (Sharpe-Schoolfield response)
# estimate all parameters
r <- nls(Development ~ xTR*(T_K/TR)*exp(A*(1/TR-1/T_K))/(1+exp(AL*(1/TL-1/T_K))+exp(AH*(1/TH-1/T_K))),
           data=sp.data, start=list(xTR=0.01, A=5000, AL=-150000, AH=200000, TL=290, TH=310))
summary(r)
# Plot model fits
plot(sp.data$T_K, sp.data$Development)
...
# estimate xTR and A
# NOTE: removed data beyond max development
r.mon <- nls(Development ~ xTR*T_K/TR*exp(A*(1/TR-1/T_K)), data=sp.data[-c((nrow(sp.data)-1):nrow(sp.data)),],
               start=list(xTR=0.1, A=1000))
summary(r.mon)
# Plot model fits
plot(sp.data$T_K, sp.data$Development)
...
# estimate AL, AH, TL, and TH
# NOTE: if needed, set AH based on NLS below and run code iteratively until AL and AH do not appreciably change)
r.SS <- nls(Development ~ coef(r.mon)[1]*(T_K/TR)*exp(coef(r.mon)[2]*(1/TR-1/T_K))/(1+exp(AL*(1/TL-1/T_K))+exp(AH*(1/TH-1/T_K))),
              data=sp.data, start=list(AL=-62000, AH=30000, TL=285.2, TH=308.1))
summary(r.SS)
# Plot model fits
plot(sp.data$T_K, sp.data$Development)
...
kTL <- 285 #290
kTH <- 305 #310
r.A <- nls(Development ~ coef(r.mon)[1]*(T_K/TR)*exp(coef(r.mon)[2]*(1/TR-1/T_K))/(1+exp(AL*(1/kTL-1/T_K))+exp(AH*(1/kTH-1/T_K))),
             data=sp.data, start=list(AL=-50000, AH=50000))
summary(r.A)
r.T <- nls(Development ~ coef(r.mon)[1]*(T_K/TR)*exp(coef(r.mon)[2]*(1/TR-1/T_K))/(1+exp(coef(r.A)[1]*(1/TL-1/T_K))+exp(coef(r.A)[2]*(1/TH-1/T_K))),
             data=sp.data, start=list(TL=kTL, TH=kTH))
summary(r.T)
# Plot model fits
plot(sp.data$T_K, sp.data$Development)#, ylim=c(0,0.07))
...
# estimate TH and AH separately from TL and AL if needed
kAL <- -100000
r.H <- nls(Development ~ coef(r.mon)[1]*(T_K/TR)*exp(coef(r.mon)[2]*(1/TR-1/T_K))/(1+exp(kAL*(1/kTL-1/T_K))+exp(AH*(1/TH-1/T_K))),
             data=sp.data, start=list(TH=kTH, AH=50000))
summary(r.H)
r.AL <- nls(Development ~ coef(r.mon)[1]*(T_K/TR)*exp(coef(r.mon)[2]*(1/TR-1/T_K))/(1+exp(AL*(1/kTL-1/T_K))+exp(coef(r.H)[2]*(1/coef(r.H)[1]-1/T_K))),
              data=sp.data, start=list(AL=-100000))
summary(r.AL)
r.TL <- nls(Development ~ coef(r.mon)[1]*(T_K/TR)*exp(coef(r.mon)[2]*(1/TR-1/T_K))/(1+exp(coef(r.AL)[1]*(1/TL-1/T_K))+exp(coef(r.H)[2]*(1/coef(r.H)[1]-1/T_K))),
              data=sp.data, start=list(TL=kTL))
summary(r.TL)
# Plot model fits
plot(sp.data$T_K, sp.data$Development)
